...
export async function handleSkaleWebhook(req: Request, res: Response) {
  try {
    const payload: SkaleWebhookPayload = req.body;

    console.log("Received Skale webhook:", JSON.stringify(payload, null, 2));
    ...
    console.error("Error processing Skale webhook:", error);
    // Return 200 anyway to prevent Skale from retrying
